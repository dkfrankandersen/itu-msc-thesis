...
impl ProductQuantization {
    ...
    fn kmeans<T: RngCore>(&self, rng: T, k_centroids: usize, max_iterations: usize, dataset: &ArrayView2::<f64>, verbose_print: bool) -> Vec::<Centroid> {
...
mod product_quantization_tests {
    ...
    fn kmeans_with_k_7_clusters_7() {
        ...
        let pq = ProductQuantization::new(false,  &dataset1().view(), 3, 1, 10, 20, 200);
        let rng = StdRng::seed_from_u64(11);
        let centroids = pq.unwrap().kmeans(rng, 7, 200, &dataset1().view(), false);
        println!("{:?}", centroids);
    ...
    fn kmeans_with_k_3_seed_111_centroid0_is() {
        use rand::{SeedableRng, rngs::StdRng};
        let pq = ProductQuantization::new(false,  &dataset1().view(), 3, 1, 10, 20, 2000);
        let rng = StdRng::seed_from_u64(111);
        let centroids = pq.unwrap().kmeans(rng, 4, 200, &dataset1().view(), false);
