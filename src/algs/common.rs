...
// impl Centroid {
//     pub fn _new(id: usize, point: Array1::<f64>) -> Self {
//         Centroid {
//             id: id,
//             point: point,
...
pub fn push_to_max_cosine_heap(heap: &mut BinaryHeap::<(OrderedFloat::<f64>, usize)>, query: &ArrayView1::<f64>, 
                                centroid_point: &ArrayView1::<f64>, centroid_index: &usize, minimum_elements: usize) {
    let distance = distance::cosine_similarity(query, centroid_point);
    if heap.len() < minimum_elements {
        heap.push((OrderedFloat(-distance), *centroid_index));
    } else if OrderedFloat(distance) > -heap.peek().unwrap().0 {
            heap.pop();
            heap.push((OrderedFloat(-distance), *centroid_index));
    }
}
