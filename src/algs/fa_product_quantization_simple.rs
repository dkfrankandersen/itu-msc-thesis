...
impl FAProductQuantization {
    ...
    pub fn new(verbose_print: bool, dist_metric: DistanceMetric, algo_parameters: &AlgoParameters,
                ...
                codebook_k: usize, max_iterations: usize) -> Result<Self, String> {
        ...
        else if codebook_k == 0 {
            return Err("codebook_k must be greater than 0".to_string());
        }
        ...
        Ok(FAProductQuantization {
            name: "fa_pq_no_residuals".to_string(),
            ...
            codebook: Array::from_elem((m, coarse_quantizer_k), Array::zeros(dataset.ncols()/m)),
            codebook_k,
    ...
    fn train_codebook(&self, training_data: &ArrayView2<f64>, m_subspaces: usize, 
                                k_centroids: usize, sub_dimension: usize) -> Array2::<Array1<f64>> {
        // Train residuals codebook
        let mut codebook = Array::from_elem((m_subspaces, k_centroids), Array::zeros(sub_dimension));
        ...
        for m in 0..m_subspaces {
            ...
            let partial_data = training_data.slice(s![.., partial_from..partial_to]);
            ...
            for (k, centroid) in centroids.iter().enumerate() {
                codebook[[m,k]] = centroid.point.clone();
            }
        }
    ...
    fn encoding(&self, residuals: &ArrayView2<f64>, codebook: &Array2::<Array1<f64>>) -> Array1<Array1<usize>> {
        // Residuals Encoding
        println!("Started encoding");
        let  mut pqcodes = Array::from_elem(residuals.nrows(), Array::from_elem(codebook.nrows(), 0));
        let bar_residuals = ProgressBar::new(residuals.nrows() as u64);
        for n in 0..residuals.nrows() {
            for m in 0..codebook.nrows() {
                ...
                for k in 0..codebook.ncols() {
                    let centroid = &codebook[[m,k]].view();
...
impl AlgorithmImpl for FAProductQuantization {
    ...
    fn fit(&mut self, dataset: &ArrayView2::<f64>) {
        ...
        if Path::new(file_compute_coarse_quantizers).exists() 
            ...
            println!("\nFit train_codebook from file");
            let mut t = DebugTimer::start("fit train_codebook from file");
            let mut read_file = BufReader::new(File::open(file_codebook).unwrap());
            ...
            let training_data = self.random_traindata(rng, dataset, self.training_size);
            t.stop();
            t.print_as_millis();

            let mut t = DebugTimer::start("fit train_codebook");
            self.codebook = self.train_codebook(&training_data.view(), self.m, 
                                                                    self.codebook_k, self.sub_dimension);
            t.stop();
            t.print_as_secs();
            ...
            self.coarse_quantizer = self.compute_coarse_quantizers(&centroids, &pq_codes, self.m);
            t.stop();
            t.print_as_secs();
    ...
    fn query(&self, dataset: &ArrayView2::<f64>,  query: &ArrayView1::<f64>, results_per_query: usize,
        ...
        let m_dim = self.codebook.nrows();
        let k_dim = self.codebook.ncols();
